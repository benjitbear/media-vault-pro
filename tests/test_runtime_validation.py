...
def load_metadata_for_file(file_path: str) -> dict:
    ...
    metadata_dir = Path('/Users/poppemacmini/Media/data/metadata')
...
class TestRuntimeValidation:
    ...
    @pytest.mark.parametrize("file_path,expected_runtime_min", _MEDIA_PAIRS,
                             ids=[Path(p).stem for p, _ in _MEDIA_PAIRS])
...
class TestRuntimeValidationUnit:
    ...
    def test_metadata_loads_for_real_files(self):
        ...
        meta_dir = Path('/Users/poppemacmini/Media/data/metadata')
        ...
        for jf in json_files:
            ...
            assert 'source_file' in data or 'tmdb' in data, (
                f"{jf.name} missing expected keys"
            )
