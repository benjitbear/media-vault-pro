...
class TestExtractMediainfo:
        ...
        mock_run.return_value = MagicMock(stdout=json.dumps({
            'media': {'track': [
                {'@type': 'General', 'Duration': '120.5',
                 'Format': 'MP4', 'FileSize': '1000000'},
                {'@type': 'Video', 'Format': 'AVC', 'Width': '1920',
                 'Height': '1080', 'FrameRate': '23.976', 'BitDepth': '8'},
                {'@type': 'Audio', 'Language': 'English', 'Format': 'AAC',
                 'Channels': '6', 'SamplingRate': '48000'},
                {'@type': 'Audio', 'Language': 'Spanish', 'Format': 'AC3',
    ...
    def test_general_track_only(self, mock_run, mock_size, client):
        mock_run.return_value = MagicMock(stdout=json.dumps({
            'media': {'track': [
                {'@type': 'General', 'Duration': '60.0',
                 'Format': 'FLAC', 'FileSize': '100'},
            ]}
        }))
