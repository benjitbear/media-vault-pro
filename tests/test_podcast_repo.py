...
class TestUpdatePodcast:
    def test_update_allowed_fields(self, app_state):
        pod_id = app_state.add_podcast(feed_url="https://example.com/feed.xml", title="Original")
    ...
    def test_update_artwork_path(self, app_state):
        pod_id = app_state.add_podcast(feed_url="https://example.com/feed.xml", title="Pod")
...
class TestDeletePodcast:
    def test_delete_existing(self, app_state):
        pod_id = app_state.add_podcast(feed_url="https://example.com/feed.xml", title="To Delete")
...
class TestGetDuePodcasts:
    def test_never_checked_is_due(self, app_state):
        app_state.add_podcast(feed_url="https://example.com/feed.xml", title="New Pod")
    ...
    def test_recently_checked_not_due(self, app_state):
        ...
        pod_id = app_state.add_podcast(feed_url="https://example.com/feed.xml", title="Checked Pod")
...
class TestUpdateEpisode:
    def test_update_file_path(self, app_state):
        pod_id = app_state.add_podcast(feed_url="https://example.com/feed.xml", title="Pod")
        ep_id = app_state.add_episode(
            podcast_id=pod_id, title="Ep 1", audio_url="https://example.com/ep1.mp3"
        )
        app_state.update_episode(ep_id, file_path="/podcasts/ep1.mp3", is_downloaded=True)
        episodes = app_state.get_episodes(pod_id)
