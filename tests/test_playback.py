...
def playback_config(tmp_path):
    ...
    return {
        "output": {
            ...
            "quality": 22, "audio_encoder": "aac", "audio_bitrate": "192"
        },
        ...
        "automation": {"auto_detect_disc": False, "auto_eject_after_rip": False,
                       "notification_enabled": False},
        "web_server": {"enabled": True, "port": 8098, "host": "127.0.0.1",
                       "library_name": "Test"},
        ...
        "uploads": {"enabled": True, "max_upload_size_mb": 10,
                    "upload_directory": str(tmp_path / "uploads")},
        "podcasts": {"enabled": False, "check_interval_hours": 6,
                     "auto_download": False,
...
def state_with_media(tmp_path):
    ...
    state.upsert_media({
        'id': 'media_001', 'title': 'Test Video', 'filename': 'test.mp4',
        'file_path': '/tmp/test.mp4', 'file_size': 1024,
        'media_type': 'video',
    })
    state.upsert_media({
        'id': 'media_002', 'title': 'Test Song', 'filename': 'song.mp3',
        'file_path': '/tmp/song.mp3', 'file_size': 512,
        'media_type': 'audio',
    })
    state.upsert_media({
        'id': 'media_003', 'title': 'Another Song', 'filename': 'song2.mp3',
        'file_path': '/tmp/song2.mp3', 'file_size': 256,
        'media_type': 'audio',
    })
    yield state
    AppState.reset()
...
class TestPlaybackProgressAPI:
    ...
    def test_save_progress_no_data(self, flask_client):
        resp = flask_client.put('/api/media/fake/progress',
                                content_type='application/json')
...
class TestChunkedStreaming:
    ...
    def test_stream_accepts_range(self, flask_client, tmp_path, playback_config):
        ...
        content, data = self._create_test_file(tmp_path, playback_config,
                                                'range_test.mp4', 1024)
        resp = flask_client.post('/api/upload', data=data,
                                 content_type='multipart/form-data')
